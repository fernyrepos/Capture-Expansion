...
namespace CaptureExpansion
{
    ...
    public static class Thing_DynamicDrawPhaseAt_Patch
    {
        public static void Postfix(Pawn __instance, DrawPhase phase)
        {
            ...
            if (bed != null && bed is not Building_Cage && __instance.TryGetData(out var data) && data.restrainedToBed)
            {
                var cornerX = (bed.def.size.x / 2f) - 0.05f;
                var cornerZ = (bed.def.size.z / 2f) - 0.05f;
                var bedCenter = bed.DrawPos;
                var pawnRot = bed.Rotation.Opposite;
                ...
                var cornerTL = bedCenter + new Vector3(-cornerX, 0f, cornerZ).RotatedBy(pawnRot);
                var cornerTR = bedCenter + new Vector3(cornerX, 0f, cornerZ).RotatedBy(pawnRot);
                var cornerBR = bedCenter + new Vector3(cornerX, 0f, -cornerZ).RotatedBy(pawnRot);
                var cornerBL = bedCenter + new Vector3(-cornerX, 0f, -cornerZ).RotatedBy(pawnRot);

                var point0 = bedCenter + off0.RotatedBy(pawnRot);
                var point1 = bedCenter + off1.RotatedBy(pawnRot);
                var point2 = bedCenter + off2.RotatedBy(pawnRot);
                var point3 = bedCenter + off3.RotatedBy(pawnRot);
